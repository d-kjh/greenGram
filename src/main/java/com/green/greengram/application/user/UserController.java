...
public class UserController {
    ...
    public ResultResponse<?> signIn(@Valid @RequestBody UserSignInReq req
        ...
        return new ResultResponse<>("sign-in success", userSignInDto.getUserSignInRes());
    }

    @PostMapping("/sign-out")
    public ResultResponse<?> signOut(HttpServletResponse response) {
        jwtTokenManager.signOut(response);
        return new ResultResponse<>("sign-out success", null);
    }
    ...
    public ResultResponse<?> patchProfilePic(@AuthenticationPrincipal UserPrincipal userPrincipal
        ...
        String savedFileName = userService.pathProfilePic(userPrincipal.getSignedUserId(), pic);
        return new ResultResponse<>("프로필 유저 정보", savedFileName);
    }
